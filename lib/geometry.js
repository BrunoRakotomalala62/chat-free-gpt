...
function tryLineByPoints(model, s) {
  ...
  const px = ensureDefinedPoint(model, X);
  const py = ensureDefinedPoint(model, Y);
  pushLine(model, { name: `(${X}${Y})`, kind: "line", from: px, to: py, pts: [X, Y], label: `droite (${X}${Y})` });
...
function tryPerp(model, s) {
  ...
  // « la perpendiculaire à (AB) en P »
  const m = s.match(/perpendiculaire\s+[àa]\s+(?:la\s+droite\s+)?\(?([A-Z]{2})\)?\s*(?:passant\s+par|en)\s+([A-Z])/)
    || s.match(/(?:passant\s+par|en)\s+([A-Z])\s+perpendiculaire\s+[àa]\s+(?:la\s+droite\s+)?\(?([A-Z]{2})\)?/);
  if (!m) return null;
  const linePts = m[1] && m[1].length === 2 ? m[1] : m[2];
  const pointName = m[1] && m[1].length === 1 ? m[1] : m[2];
  if (!linePts || !pointName) return null;
  const [X, Y] = [linePts[0], linePts[1]];
  const px = ensureDefinedPoint(model, X);
  const py = ensureDefinedPoint(model, Y);
  ...
  pushLine(model, { name, kind: "line", from: pp, to: q, pts: [pointName], label: `${name} \u22a5 (${X}${Y}) en ${pointName}` });
  pushMark(model, { at: pp, u: { x: dx, y: dy }, v: { x: -dy, y: dx } });
  return { kind: "perpendiculaire", label: `${name} perpendiculaire \u00e0 (${X}${Y}) en ${pointName}` };
}

function tryParallel(model, s) {
  // « parallèle à (AB) passant par P », « la droite passant par P parallèle à (AB) »
  const m = s.match(/parall[eè]le\s+[àa]\s+(?:la\s+droite\s+)?\(?([A-Z]{2})\)?\s*(?:passant\s+par|en)\s+([A-Z])/)
    || s.match(/(?:passant\s+par|en)\s+([A-Z])\s+parall[eè]le\s+[àa]\s+(?:la\s+droite\s+)?\(?([A-Z]{2})\)?/);
  if (!m) return null;
  const linePts = m[1] && m[1].length === 2 ? m[1] : m[2];
  const pointName = m[1] && m[1].length === 1 ? m[1] : m[2];
  if (!linePts || !pointName) return null;
  ...
  const name = nextLineName(model);
  pushLine(model, { name, kind: "line", from: pp, to: q, pts: [pointName], label: `${name} \u2225 (${X}${Y})` });
  return { kind: "parallèle", label: `${name} parall\u00e8le \u00e0 (${X}${Y}) en ${pointName}` };
}
