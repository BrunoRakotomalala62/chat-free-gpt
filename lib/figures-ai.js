...
const { chatReliable, isSupportedModel } = require("./aichatting");

const clamp = (v, min, max) => Math.min(max, Math.max(min, v));
const isHexColor = (c) => /^#[0-9a-fA-F]{6}$/.test(String(c || ""));

function buildPrompt(subject, width, height, color, strict, kind) {
  ...
  const role =
    kind === "geo"
      ? "Tu es un dessinateur de figures de GÉOMÉTRIE (règle, équerre, compas).\n" +
        `Sujet : « ${subject} »\n` +
        `Réponds UNIQUEMENT avec le code SVG complet <svg>…</svg> — aucun texte autour, aucun markdown.\n` +
        `Règles : fond blanc ; dessine la construction géométrique demandée avec précision : ` +
        `points nommés en majuscules (A, B, C…), droites (AB), segments [AB], demi-droites, cercles ` +
        `(centre et rayon), angles (arc + valeur si donnée), marques d'angle droit et d'égalité de longueur ; ` +
        `légende chaque élément avec <text> en français (police sans-serif) ; ` +
        `tout doit tenir dans le viewBox ${width}x${height} ; interdit : <script>, javascript:, attributs on*.\n` +
...
/** Chaîne de repli : retente la génération avec un visiteur neuf en cas d'échec
 *  (504, délai, SVG invalide). Un seul modèle : le backend aichatting ne fait
 *  tourner qu'un moteur gratuit (gpt-5.6-luna = ChatGPT) — les autres noms
 *  testés retombaient silencieusement sur lui (vérifié le 2026-09-05). */
const MODEL_CHAIN = ["gpt-5.6-luna"];
 ...
 * @param {object} opts { subject, width, height, model, color, budgetMs }
 *   budgetMs : budget total en ms (défaut BUDGET_MS) — utile quand la figure
 *   IA est produite dans une route déjà partiellement consommée (vérification).
...
async function buildFigureFromSubject(opts = {}) {
  ...
  // Les noms inconnus (anciens alias) sont ignorés : ils retombaient sur le
  // même moteur par défaut, on ne les laisse plus passer comme « modèle ».
  const userModel = String(opts.model || "").trim();
  const candidates = [];
  for (const m of [userModel, ...MODEL_CHAIN]) {
    if (m && isSupportedModel(m) && !candidates.includes(m)) candidates.push(m);
  }

  const budgetMs = Number(opts.budgetMs) > 0 ? Number(opts.budgetMs) : BUDGET_MS;
  const deadline = Date.now() + budgetMs;
  let lastError = "aucune information";
  let totalTries = 0;

  const kind = opts.kind === "geo" ? "geo" : null;
    ...
    while (attempt <= 2) {
      ...
      try {
        const result = await withTimeout(
          chatReliable({
            prompt: buildPrompt(subject, width, height, color, attempt === 2, kind),
            model,
            lang: "fr",
          }),
          timeoutMs
...
module.exports = { buildFigureFromSubject, extractSvg, sanitizeSvg, isWellFormedXml, withTimeout };
