...
const { chatReliable, FREE_MODELS, PRO_MODELS, isSupportedModel } = require("./aichatting");
const { buildPromptWithContext, rememberExchange } = require("./conversation");
...
async function handleChatRequest(req, res) {
  ...
  const uid = get("uid");
  const lang = get("lang");
  const history = get("history"); // JSON [{q, a}, …] fourni par le client (optionnel)
  const reset = get("reset") === "1" || get("reset") === "true"; // force nouveau sujet
  let images = [];
  if (query) {
  ...
  // Seuls les modèles réellement supportés par le backend sont acceptés :
  // les noms inconnus ne retombent plus silencieusement sur le modèle par
  // défaut (ils faisaient croire à un modèle qui ne tournait pas).
  if (model && !isSupportedModel(String(model).trim())) {
    json(
      res,
      400,
      {
        success: false,
        error: `Modèle inconnu : « ${model} ». Ce nom n'existe pas sur le backend (les noms non listés répondaient en réalité via le modèle par défaut).`,
        models_disponibles: FREE_MODELS,
        models_pro: PRO_MODELS,
        hint: "Utilisez un modèle de la liste, ou omettez 'model' pour le modèle gratuit par défaut.",
      },
      cors
    );
    return;
  ...
  try {
    // --- Conversation continue : enchaîne sur l'échange précédent (texte ET
    // vision), ou repart de zéro si l'utilisateur change de thème. ---
    const rawPrompt = prompt ? String(prompt).trim() : "";
    const ctx = buildPromptWithContext({
    ...
    // Mémorise l'échange (question propre + réponse) pour la suite —
    // sauf en cas d'erreur PRO/quota : on ne pollue pas la mémoire.
    if (!isProOnly && result.reply && result.reply.trim() && uid !== undefined && uid !== null) {
      rememberExchange(
        String(uid),
        rawPrompt + (images.length ? "\n[Image(s) jointe(s) par l'utilisateur à cette question.]" : ""),
        result.reply
      );
    }
    ...
    json(
      ...
      isProOnly ? 402 : 200,
      {
        ...
        context: ctx.kind, // "none" | "continued" | "new_topic"
        source: "https://www.aichatting.net/fr/free-chatgpt/",
        ...(isProOnly
          ? {
              error:
                "Ce modèle nécessite un compte PRO (aichatting.net). Essayez le modèle gratuit : gpt-5.6-luna.",
            }
          : {}),
      },
      cors
