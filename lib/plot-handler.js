...
async function handlePlotRequest(req, res) {
  ...
  if (!expression && !subject) {
    json(
      ...
      {
        success: false,
        error: "Le paramètre 'expression' (courbe mathématique) ou 'subject' (figure par IA) est obligatoire.",
  ...
  try {
    ...
    json(
      ...
      200,
      {
        ...
        affichage: "La figure est dans le champ 'svg' (à injecter dans le DOM ou à convertir en PNG). Les asymptotes et branches infinies sont détectées automatiquement ; la tangente n'est tracée que si le paramètre 'tangent' est fourni.",
      },
      cors
    );
  } catch (err) {
