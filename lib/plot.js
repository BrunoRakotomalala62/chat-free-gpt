...
/** L'AST contient-il la variable x ? */
function astContainsVar(ast) {
  switch (ast.type) {
    case "var": return true;
    case "num": return false;
    case "neg": return astContainsVar(ast.operand);
    case "bin": return astContainsVar(ast.left) || astContainsVar(ast.right);
    case "call": return ast.args.some(astContainsVar);
    default: return false;
  }
}

/**
 * L'AST est-il une droite affine y = a·x + b ?
 * (linéaire en x : pas de x², pas de fonction, pas de produit de termes en x…)
 */
function isLinearAst(ast) {
  switch (ast.type) {
    case "num": return true;
    case "var": return true;
    case "neg": return isLinearAst(ast.operand);
    case "bin": {
      if (ast.op === "+" || ast.op === "-") return isLinearAst(ast.left) && isLinearAst(ast.right);
      if (ast.op === "*" || ast.op === "/") {
        // au plus un des deux côtés contient x (sinon x², x/x…)
        const lv = astContainsVar(ast.left);
        const rv = astContainsVar(ast.right);
        if (lv && rv) return false;
        return isLinearAst(ast.left) && isLinearAst(ast.right);
      }
      if (ast.op === "^") {
        // x^1 ou x^0 uniquement
        if (ast.right.type === "num") {
          const e = ast.right.value;
          if (e === 1) return isLinearAst(ast.left);
          if (e === 0) return !astContainsVar(ast.left);
        }
        return false;
      }
      return false;
    }
    case "call": return false; // sin(x), ln(x)… : pas une droite
    default: return false;
  }
}
...
/** Équation affine "y = mx + b" propre (ex. "y = 2x - 3", "y = x + 1", "y = 1"). */
function formatAffine(m, b) {
  const mr = Math.round(m * 100) / 100;
  if (Math.abs(mr) < 1e-9) return `y = ${fmtNum(Math.round(b * 100) / 100)}`; // pente nulle
  let s = "y = ";
 ...
 * @param {object} opts { expression, xmin, xmax, ymin, ymax, width, height, samples, color, title, tangent, line }
 *   tangent : abscisse x0 où tracer la tangente (optionnel — rien sans cela)
 *   line    : droite donnée directement par l'exercice, ex. "2x-3" ou "y=2x-3"
 *             (tracée en vert ; sans `expression`, elle devient la figure)
 * @returns {object} { svg, points, expression, domain, range, size, samples, asymptotes, branches, tangente, droite }
 */
function buildFigure(opts = {}) {
  ...
  // droite donnée directement (y = ax+b) : validation + coefficient
  let lineAst = null;
  let lineDisplay = "";
  let lineCoefs = null;
  if (optLine !== undefined && optLine !== null && String(optLine).trim() !== "") {
    const lineSrc = String(optLine).trim();
    const { ast: la, display: ld } = parseExpression(lineSrc);
    if (!isLinearAst(la)) {
      throw new Error(`"${ld}" n'est pas une droite affine y = ax + b.`);
    }
    lineAst = la;
    lineDisplay = ld;
    const y0 = evaluate(la, 0);
    const y1 = evaluate(la, 1);
    if (!isNum(y0) || !isNum(y1)) throw new Error("Droite invalide.");
    lineCoefs = { a: y1 - y0, b: y0 };
  }

  const expr = expression || lineDisplay; // sans fonction, on trace la droite seule
  if (!expr) throw new Error("Paramètre 'expression' ou 'line' manquant.");

  const { ast, display } = parseExpression(expr);
  ...
  const hasOwnExpression = !!(expression && String(expression).trim());

  // ---- analyse des branches infinies (asymptotes) et tangente ----
  // (uniquement si une vraie fonction est tracée ; une droite seule n'a pas
  //  d'asymptote — elle serait sinon détectée comme « oblique » d'elle-même)
  let verticales = [];
  let horizontales = [];
  let obliques = [];
  let branches = [];
  let tangent = null;
  if (hasOwnExpression) {
    verticales = detectVerticalAsymptotes(ast, xmin, xmax);
    const ana = analyzeBranches(ast);
    horizontales = ana.horizontales;
    obliques = ana.obliques;
    branches = ana.branches;
  ...
  // ---- droite d'équation y = ax+b donnée directement (verte) ----
  const lineColor = "#16a34a";
  if (lineAst) {
    const clip = clipSegment(
      px(xmin), py(evaluate(lineAst, xmin)),
      px(xmax), py(evaluate(lineAst, xmax)),
      M.left, M.top, M.left + plotW, M.top + plotH
    );
    if (clip) {
      extraSvg += `<line x1="${fmtPx(clip[0].x)}" y1="${fmtPx(clip[0].y)}" x2="${fmtPx(clip[1].x)}" y2="${fmtPx(clip[1].y)}" stroke="${lineColor}" stroke-width="2.2"/>`;
    }
  }

  // ---- légende : branches infinies / asymptotes / tangente / droite ----
  const legendEntries = [];
  if (lineAst) {
    legendEntries.push({ text: `Droite : ${formatAffine(lineCoefs.a, lineCoefs.b)}`, color: lineColor });
  }
  for (const x0 of verticales) {
  ...
  const title = optTitle
    ...
    : hasOwnExpression
      ? `f(x) = ${display}`
      : `Droite (d) : ${formatAffine(lineCoefs.a, lineCoefs.b)}`;
  ...
  return {
    ...
    tangente: tangent,
    droite: lineCoefs
      ? { equation: formatAffine(lineCoefs.a, lineCoefs.b), a: lineCoefs.a, b: lineCoefs.b, display: lineDisplay }
      : null,
  };
}
